function showHuaweiTable()
{
    ...
    huaweiTable = new Tabulator("#Main-mbb-table", {
        layout:"fitDataFill",
        tooltips:true,
        addRowPos:"top",
        history:false,
    ...
    $.ajax({
        type: "post",
        url: api_url+'getHuawei.php',
        data:{Username:Username,isAdmin:isAdmin},
        dataType:'JSON',
        success: function(data){
            huaweiTable.setData(data);
        }
    });
    ...
    $("#Main-mbb-columns").on("change", function(){
        ...
        if (column==null) {
            huaweiTable.clearFilter(true);
            return;
        }
        var input=$("#Main-mbb-controls input[name=name]").val();
        huaweiTable.setFilter(column, "like", input);
        $("#Main-mbb-table-count").text(huaweiTable.getDataCount());
    });
    $("#Main-mbb-controls input[name=name]").on("keyup", function(){
        ...
        huaweiTable.setFilter(column, "like", $(this).val());
        $("#Main-mbb-table-count").text(huaweiTable.getDataCount());
    });
