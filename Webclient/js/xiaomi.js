function showXiaomiTable()
{
    ...
    xiaomiTable = new Tabulator("#Main-mbb-table", {
        layout:"fitDataFill",
        tooltips:true,
        addRowPos:"top",
        history:false,
    ...
    $.ajax({
        type: "post",
        url: api_url+'getXiaomi.php',
        data:{Username:Username,isAdmin:isAdmin},
        dataType:'JSON',
        success: function(data){
            xiaomiTable.setData(data);
        }
    });
    ...
    $("#Main-mbb-columns").on("change", function(){
        ...
        if (column==null) {
            xiaomiTable.clearFilter(true);
            return;
        }
        var input=$("#Main-mbb-controls input[name=name]").val();
        xiaomiTable.setFilter(column, "like", input);
        $("#Main-mbb-table-count").text(xiaomiTable.getDataCount());
    });
    $("#Main-mbb-controls input[name=name]").on("keyup", function(){
        ...
        xiaomiTable.setFilter(column, "like", $(this).val());
        $("#Main-mbb-table-count").text(xiaomiTable.getDataCount());
    });
