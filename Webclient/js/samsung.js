function showSamsungTable()
{
    ...
    samsungTable = new Tabulator("#Main-mbb-table", {
        layout:"fitDataFill",
        tooltips:true,
        addRowPos:"top",
        history:false,
    ...
    $.ajax({
        type: "post",
        url: api_url+'getSamsung.php',
        data:{Username:Username,isAdmin:isAdmin},
        dataType:'JSON',
        success: function(data){
            samsungTable.setData(data);
        }
    });
    ...
    $("#Main-mbb-columns").on("change", function(){
        ...
        if (column==null) {
            samsungTable.clearFilter(true);
            return;
        }
        var input=$("#Main-mbb-controls input[name=name]").val();
        samsungTable.setFilter(column, "like", input);
        $("#Main-mbb-table-count").text(samsungTable.getDataCount());
    });
    $("#Main-mbb-controls input[name=name]").on("keyup", function(){
        ...
        samsungTable.setFilter(column, "like", $(this).val());
        $("#Main-mbb-table-count").text(samsungTable.getDataCount());
    });
